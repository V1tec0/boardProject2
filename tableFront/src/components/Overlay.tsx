...
//         const interval = setInterval(checkBellSchedule, 20000);
...
const Overlay: React.FC = () => {
    const { isServerAlive } = useServerStatus();
    const [currentMessage, setCurrentMessage] = useState<IMessage | null>(null);
    const [currentBell, setCurrentBell] = useState<BellSchedule | null>(null);
    const [messageModalVisible, setMessageModalVisible] = useState(false);
    const [bellModalVisible, setBellModalVisible] = useState(false);
    ...
    useEffect(() => {
        if (!isServerAlive) {
            message.error('Сервер недоступен, остановка запросов расписания.');
            return;
        }
        ...
        const checkBellSchedule = async () => {
            try {
                ...
                const now = moment();
                const currentTimeStr = now.format('HH:mm');
                const currentDateTimeKey = now.format('YYYY-MM-DD HH:mm');
